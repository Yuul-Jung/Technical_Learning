...
module basys3_uart_rx_tx_top #(
    ...
    // ------------------------------------------------------------
    // (변경) RX 표시 Register
    // - rx_valid가 발생하면 수신 1문자를 즉시 LED 표시용 Register에 저장
    // - line_frame_valid는 LF 기준 Frame 완료 표시만 담당
    // ------------------------------------------------------------
    always @(posedge clk) begin
        if (reset) begin
            ...
            // (변경) 수신 데이터가 유효하면 LF를 기다리지 않고 즉시 표시
            if (rx_valid) begin
                display_last_data <= rx_data;
            end

            // LF를 만나 Frame이 완료되면 Frame 완료 상태만 저장
